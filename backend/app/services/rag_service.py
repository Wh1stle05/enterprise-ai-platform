...
NO_EVIDENCE_MESSAGE = "No sufficient evidence found. Please add relevant documents or contact an administrator."
...
async def answer_question(
    ...
    if not evidence:
        ...
        return answer
    sources = "\n\n".join(
        f"[S{i}] filename={hit.filename} locator={hit.source_locator} chunk_id={hit.chunk_id}\n{hit.content}"
        for i, hit in enumerate(evidence, 1)
    )
    answer_text = await completer([
        {"role": "system", "content": "Answer only from the supplied sources. Cite every factual claim using supplied labels. Never invent labels. If evidence is insufficient return the exact fallback."},
    ...
    await record_audit(db, action_type="knowledge_query", user_id=user.id, input_data=question,
                       output={"kb_id": kb_id, "chunks": [{"chunk_id": h.chunk_id, "score": h.score} for h in hits],
                               "cited_chunk_ids": [c.chunk_id for c in citations],
                               "citation_coverage": 1.0, "no_evidence": False}, tool_used="knowledge_rag")
