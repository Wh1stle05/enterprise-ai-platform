...
async def make_run(db_session, *, allowed_tools=("read_item", "write_item", "fail_item")):
    user = User(
        id=uuid4(), username=f"u-{uuid4().hex[:8]}", email=f"{uuid4().hex[:8]}@example.com",
        hashed_password="hash",
    )
    conversation = Conversation(id=uuid4(), user_id=user.id, title="test")
    run = AgentRun(
        id=uuid4(), conversation_id=conversation.id, user_id=user.id,
        allowed_tools=json.dumps(list(allowed_tools)), status="running",
    )
...
async def test_immediate_read_executes_with_exact_arguments_and_audit(db_session):
    ...
    call = await propose_or_execute(
        db_session, registry, run_id=run.id, user_id=user.id,
        planned_call=PlannedToolCall("provider-1", "read_item", arguments), step_number=3, now=NOW,
    )
...
async def test_write_pauses_without_handler_and_persists_exact_impact_and_expiry(db_session):
    ...
    call = await propose_or_execute(
        db_session, registry, run_id=run.id, user_id=user.id,
        planned_call=PlannedToolCall("provider-2", "write_item", arguments), step_number=4, now=NOW,
...
async def test_whitelist_and_validation_errors_are_rejected(db_session):
    ...
    with pytest.raises(HTTPException) as unknown:
        await propose_or_execute(
            db_session, registry, run_id=run.id, user_id=user.id,
            planned_call=PlannedToolCall("x", "write_item", {"item": "A"}), now=NOW,
        )
    ...
    with pytest.raises(HTTPException) as invalid:
        await propose_or_execute(
            db_session, registry, run_id=run.id, user_id=user.id,
            planned_call=PlannedToolCall("x", "read_item", {"extra": True}), now=NOW,
        )
...
async def test_owner_foreign_ids_are_404(db_session):
    ...
    with pytest.raises(HTTPException) as error:
        await propose_or_execute(
            db_session, registry, run_id=run.id, user_id=other,
            planned_call=PlannedToolCall("x", "read_item", {"item": "A"}), now=NOW,
        )
...
async def test_confirm_claims_exactly_once_and_repeat_is_conflict(db_session):
    ...
    executed = await decide_tool_call(
        db_session, registry, tool_call_id=pending.id, user_id=user.id, confirm=True, now=NOW,
    )
...
async def test_deny_expiry_and_handler_failure_are_terminal_and_audited(db_session):
    ...
    denied = await propose_or_execute(
        db_session, registry, run_id=run.id, user_id=user.id,
        planned_call=PlannedToolCall("deny", "write_item", {"item": "D"}), now=NOW,
    )
    denied = await decide_tool_call(
        db_session, registry, tool_call_id=denied.id, user_id=user.id, confirm=False, now=NOW,
    )
    assert denied.status == "denied"

    expired = await propose_or_execute(
        db_session, registry, run_id=run.id, user_id=user.id,
        planned_call=PlannedToolCall("expire", "write_item", {"item": "E"}), step_number=2,
        now=NOW, confirmation_ttl=timedelta(seconds=1),
    )
    expired = await decide_tool_call(
        db_session, registry, tool_call_id=expired.id, user_id=user.id, confirm=True,
    ...
    failing = await propose_or_execute(
        db_session, registry, run_id=run.id, user_id=user.id,
        planned_call=PlannedToolCall("fail", "fail_item", {}), step_number=3, now=NOW,
    )
    failing = await decide_tool_call(
        db_session, registry, tool_call_id=failing.id, user_id=user.id, confirm=True, now=NOW,
    )
